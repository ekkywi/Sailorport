...
| Webhook: tanpa dedupe `X-GitHub-Delivery` + tanpa rate limit | Sedang | **Dedupe:** ✅ Step 33a–b (`webhook_deliveries`). **Rate limit webhook** masih debt. Unit test replay → Step 33c |
...
| 2026-09-23 | Replay webhook GitHub bisa buat banyak deployment untuk delivery yang sama | **Step 33a–b:** tabel `webhook_deliveries` + skip Create jika `X-GitHub-Delivery` sudah tercatat |
