...
17. **Worker admin lite (Step 28):** Admin PATCH worker labels; Decommission → `draining`; heartbeat tetap draining; deploy dengan `worker_id` draining → **409**; Restore → `offline`. Portal Workers (admin): Edit labels / Decommission / Restore; non-admin tanpa Actions (`docs/PROGRESS.md` tes 28c–28e).
...
| Portal masih menampilkan form `/register` padahal register sudah tertutup | — | ✅ Fixed Step 30e — route/link `/register` dihapus; first-run lewat `/setup` |
| `model.RegisterRequest.Role` diabaikan | Rendah | Legacy field; `POST /auth/register` selalu 403 (Step 30e). Bootstrap pakai `SetupAdminRequest` tanpa role |
...
| 2026-09-14 | Bootstrap masih lewat `/register`; portal menampilkan Sign up | **Step 30:** first-run `/setup` + gate; `POST /auth/register` selalu **403**; link/route `/register` dihapus |
